...
const boost::dynamic_bitset<>& NearNeighboursData::get_vertices_at_exact_distance(
  ...
  if(list.empty()) {
    ...
    for(const auto& vertex_weight_pair : m_ndata.get_neighbours_and_weights(v)) {
  ...
  while(index >= list.size()) {
    ...
    for(current_bitset.resize(m_data.size()); 
              vertex < m_data.size();
              vertex = previous_bitset.find_next(vertex)) {
      for(const auto& vertex_weight_pair : m_ndata.get_neighbours_and_weights(vertex)) {
...
static void fill_degree_counts_vector(std::vector<std::size_t>& degree_counts_work_vector,
...
const FilterUtils::DegreeCounts& NearNeighboursData::get_degree_counts_at_exact_distance(
      VertexWSM v, unsigned distance) {
  std::vector<FilterUtils::DegreeCounts>& list = m_data.at(v).degree_counts_for_exact_distance;
  TKET_ASSERT(distance > 0);
  const unsigned index = distance-1;
  if(index < list.size()) {
    return list[index];
  ...
  for(unsigned distance_for_calculation=list.size()+1;
          distance_for_calculation <= distance;
          ++distance_for_calculation) {
    TKET_ASSERT(list.size()+1 == distance_for_calculation);
    list.resize(distance_for_calculation);

    fill_degrees_vector(
          get_vertices_at_exact_distance(v, distance_for_calculation),
          m_ndata,
...
const FilterUtils::DegreeCounts& NearNeighboursData::get_degree_counts_up_to_distance(
      VertexWSM v, unsigned distance) {
  ...
  while(index >= list.size()) {
    ...
    list.emplace_back();
    fill_degrees_vector(get_vertices_up_to_distance(v, list.size()),
          m_ndata, m_degree_counts_work_vector);
