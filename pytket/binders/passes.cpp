...
Transform lightsabre_transform(
  ...
  return Transform([=](Circuit &circ) {
    py::module lightsabre_module =
        py::module::import("pytket.passes.lightsabre");
...
void from_json(const nlohmann::json &j, PassPtr &pp) {
  ...
  if (classname == "StandardPass") {
    ...
    } else if (passname == "LightSABREPass") {
      Architecture arc = content.at("architecture").get<Architecture>();
      unsigned seed = content.at("seed").get<unsigned>();
      unsigned level = content.at("optimisation_level").get<unsigned>();
      pp = lightsabre_routing(arc, seed, level);
