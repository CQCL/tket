...
class BasePass:
    ...
    def apply(
        self,
        compilation_unit: pytket._tket.predicates.CompilationUnit,
    ...
    def apply(
        ...
        before_apply: Callable[[pytket._tket.predicates.CompilationUnit, object], None],
        after_apply: Callable[[pytket._tket.predicates.CompilationUnit, object], None],
    ) -> bool: ...
    @classmethod
...
class RepeatUntilSatisfiedPass(BasePass):
    @overload
    def __init__(
        self, compilation_pass: BasePass, predicate: pytket._tket.predicates.Predicate
    ) -> None: ...
    @overload
    def __init__(
        self,
        compilation_pass: BasePass,
        check_function: Callable[[pytket._tket.circuit.Circuit], bool],
    ) -> None: ...
    def get_pass(self) -> BasePass: ...
...
class RepeatWithMetricPass(BasePass):
    def __init__(
        self,
        compilation_pass: BasePass,
        metric: Callable[[pytket._tket.circuit.Circuit], int],
    ) -> None: ...
...
def AASRouting(
    arc: pytket._tket.architecture.Architecture, **kwargs: Any
) -> BasePass: ...
def CXMappingPass(
    arc: pytket._tket.architecture.Architecture,
    placer: pytket._tket.placement.Placement,
    **kwargs: Any
) -> BasePass: ...
...
def ContextSimp(
    allow_classical: bool = ..., xcirc: Optional[pytket._tket.circuit.Circuit] = ...
) -> BasePass: ...
def CustomPass(
    transform: Callable[[pytket._tket.circuit.Circuit], pytket._tket.circuit.Circuit],
    label: str = ...,
) -> BasePass: ...
def CustomRoutingPass(
    arc: pytket._tket.architecture.Architecture,
    config: List[pytket._tket.mapping.RoutingMethod],
) -> BasePass: ...
...
def DecomposeSwapsToCXs(
    arc: pytket._tket.architecture.Architecture, respect_direction: bool = ...
) -> BasePass: ...
def DecomposeSwapsToCircuit(
    replacement_circuit: pytket._tket.circuit.Circuit,
) -> BasePass: ...
def DecomposeTK2(allow_swaps: bool = ..., **kwargs: Any) -> BasePass: ...
def DefaultMappingPass(
    arc: pytket._tket.architecture.Architecture, delay_measures: bool = ...
) -> BasePass: ...
def DelayMeasures(allow_partial: bool = ...) -> BasePass: ...
def EulerAngleReduction(
    q: pytket._tket.circuit.OpType, p: pytket._tket.circuit.OpType, strict: bool = ...
) -> BasePass: ...
...
def FullPeepholeOptimise(
    allow_swaps: bool = ..., target_2qb_gate: pytket._tket.circuit.OpType = ...
) -> BasePass: ...
def GlobalisePhasedX(squash: bool = ...) -> BasePass: ...
def GuidedPauliSimp(
    strat: pytket._tket.transform.PauliSynthStrat = ...,
    cx_config: pytket._tket.circuit.CXConfigType = ...,
) -> BasePass: ...
@overload
def KAKDecomposition(
    target_2qb_gate: pytket._tket.circuit.OpType = ...,
    cx_fidelity: float = ...,
    allow_swaps: bool = ...,
) -> BasePass: ...
@overload
...
def OptimisePhaseGadgets(
    cx_config: pytket._tket.circuit.CXConfigType = ...,
) -> BasePass: ...
def PauliExponentials(
    strat: pytket._tket.transform.PauliSynthStrat = ...,
    cx_config: pytket._tket.circuit.CXConfigType = ...,
) -> BasePass: ...
def PauliSimp(
...
@overload
def RebaseCustom(
    gateset: set[pytket._tket.circuit.OpType],
    cx_replacement: pytket._tket.circuit.Circuit,
    tk1_replacement: Callable[
        [Union[sympy.Expr, float], Union[sympy.Expr, float], Union[sympy.Expr, float]],
...
def RenameQubitsPass(
    qubit_map: Dict[pytket._tket.unit_id.Qubit, pytket._tket.unit_id.Qubit]
) -> BasePass: ...
...
def SimplifyInitial(
    ...
    create_all_qubits: bool = ...,
    remove_redundancies: bool = ...,
    xcirc: pytket._tket.circuit.Circuit = ...,
) -> BasePass: ...
def SimplifyMeasured() -> BasePass: ...
def SquashCustom(
    singleqs: set[pytket._tket.circuit.OpType],
    tk1_replacement: Callable[
        [Union[sympy.Expr, float], Union[sympy.Expr, float], Union[sympy.Expr, float]],
        pytket._tket.circuit.Circuit,
    ],
    always_squash_symbols: bool = ...,
) -> BasePass: ...
