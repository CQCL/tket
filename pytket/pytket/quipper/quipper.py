...
class QuipperTransformer(Transformer):
    ...
    def qgate(self, t: list) -> QGate:  # noqa: PLR0912
        ops = QGate_Op
        n = t[0]
        if n == "not" or n == "x" or n == "X":  # noqa: PLR1714
            op = ops.Not
        elif n == "H":
            op = ops.H
        elif n == "multinot":
    ...
    def control_app(self, t: list) -> Control:
        if not t:
            return Control(controlled=list(), no_control=False)  # noqa: C408
        if len(t) == 2:  # noqa: PLR2004
            return Control(controlled=t[0], no_control=True)
        if t[0] == "with nocontrol":
            return Control(controlled=list(), no_control=True)  # noqa: C408
...
class CircuitMaker:
    def __init__(self, subr: list[Subroutine]) -> None:
        self.subrd = dict((s.name, s) for s in subr)  # noqa: C402
    ...
    def make_circuit(self, circ: Program) -> Circuit:  # noqa: PLR0912, PLR0915
        ...
        tkqbits = dict((qbits[i], i) for i in range(n_qbits))  # noqa: C402
        tkcbits = dict((cbits[i], i) for i in range(n_cbits))  # noqa: C402
        ...
        for gate in gates:
                ...
                if op == "Not":
                    ...
                    for wire in wires:
                        if n_ctrls == 0:
                            ...
                            c.add_gate(OpType.CnX, qctrls + [wire])  # noqa: RUF005
                elif op == "H":
                    if n_ctrls == 0:
                        c.H(wires[0])
                    elif n_ctrls == 1:
                    ...
                    raise TypeError("Unknown op type: %s" % op)  # noqa: UP031
            ...
            # Not supported in pytket.
                ...
                raise TypeError("Unknown gate type: %s" % type(gate))  # noqa: UP031
        return c
