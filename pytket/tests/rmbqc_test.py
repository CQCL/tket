...
from pytket.rmbqc import (
    MPattern,
    Splitter,
    Knitter,
    repeated_mbqc_conversion,
    is_mbqc_clifford,
    count_nCliffords,
    depth_structure,
)
...
def test_zx_diagram() -> None:
    c = Circuit(4)
    c.H(0).T(2).CZ(0, 1).CX(0, 2).X(1).H(2).T(2).CZ(1, 2).CX(1, 0).T(0).T(1).CZ(
        0, 2
    ).CZ(1, 3).T(0).X(1).T(2).T(3)
    mp = MPattern(c)
...
def test_layer_list() -> None:
    ...
    assert not len(MPattern.layer_list(gf_list[0])) == len(
        MPattern.layer_list(gf_list[1])
    )
...
def test_unrouted() -> None:
    c = Circuit(3)
    c.H(0).T(2).CZ(0, 1).CX(0, 2).X(1).H(2).T(2).CZ(1, 2).CX(1, 0).T(0).T(1).CZ(0, 2).T(
        0
    ).X(1).T(2)
...
def test_sequential() -> None:
    ...
    assert final_inputs == {
        Qubit(0): arch.nodes[5],
        Qubit(1): arch.nodes[0],
        Qubit(2): arch.nodes[2],
    }
    assert final_outputs == {
        Qubit(0): arch.nodes[7],
        Qubit(1): arch.nodes[3],
        Qubit(2): arch.nodes[4],
    }
...
def test_separate() -> None:
    ...
    assert final_outputs == {
        Qubit(0): arch.nodes[0],
        Qubit(1): arch.nodes[1],
        Qubit(2): arch.nodes[3],
    }
    assert final_c.depth() == 35
    assert final_c.n_qubits == 9
...
def test_repeated_mbqc_conversion() -> None:
    ...
    arch = SquareGrid(3, 3)
    (final_c, final_inputs, final_outputs) = repeated_mbqc_conversion(
        c, arch, 3, Splitter.depth_split, Knitter.separate
    )
    assert final_inputs == {
        Qubit(0): arch.nodes[5],
        Qubit(1): arch.nodes[0],
