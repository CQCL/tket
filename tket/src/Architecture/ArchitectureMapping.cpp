  ...
  for (size_t ii = 0; ii < m_vertex_to_node_mapping.size(); ++ii) {
    const auto& node = m_vertex_to_node_mapping[ii];
    {
      ...
      TKET_ASSERT_WITH_MESSAGE(
          ...
          std::stringstream()
              << "Duplicate node " << node.repr() << " at vertices "
              << citer->second << ", " << ii);
      // GCOVR_EXCL_STOP
    }
...
ArchitectureMapping::ArchitectureMapping(
  ...
  TKET_ASSERT_WITH_MESSAGE(
      ...
      std::stringstream() << "passed in " << edges.size() << " edges, giving "
...
const Node& ArchitectureMapping::get_node(size_t vertex) const {
  ...
  // GCOVR_EXCL_START
  TKET_ASSERT_WITH_MESSAGE(
      vertex < num_vertices, std::stringstream()
...
size_t ArchitectureMapping::get_vertex(const Node& node) const {
  ...
  TKET_ASSERT_WITH_MESSAGE(
      citer != m_node_to_vertex_mapping.cend(),
      std::stringstream() << "node " << node.repr() << " has no vertex number");
  // GCOVR_EXCL_STOP
  return citer->second;
}
