...
Subcircuit QInteraction::subcircuit() const { return {in_edges_, out_edges_, vertices_}; }
...
QISystem::QISystem(Circuit &circ, std::function<Circuit(Circuit)> replacement_func)
		: circ_(circ), bin_(), interactions_(), idx_(0), replacement_func_(replacement_func) {}
...
std::pair<bool, EdgeVec> QISystem::close_interaction(int i) {
...
bool QISystem::close_interaction_and_spawn(int i) {
...
bool QISystem::close_interactions_feeding_vertex(const Vertex &v) {
	bool changed = false;
...
bool QISystem::close_all_interactions() {
	bool changed = false;
	// Form set of keys.
	std::set<int> indices;
