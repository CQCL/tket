...
struct ChoiMixBuilder {
  ...
  // TODO:: Try combining solutions for postselected, collapsed, and initialised
  // subspaces into a single diagonalisation problem; rewrite method desciption
  // in Converters.hpp

  // Identify and solve all post-selections by Gaussian elimination and
  // diagonalisation of the input-only rows
  ...
  // After removing post-selected subspace and identity subspace, all remaining
  // rows with non-trivial input segment commute over the inputs; each row of
  // these is reduced to a single qubit which is collapsed, so it only carries
  // one stabiliser generator rather than the usual pair
  void solve_collapse_subspace();
  // Solve all zero-initialisations by Gaussian elimination and diagonalisation
  // of the output-only rows
...
std::pair<Circuit, qubit_map_t> cm_tableau_to_exact_circuit(
  ...
  // std::cout << builder.realised_tableau() << std::endl;
  ...
  // std::cout << builder.realised_tableau() << std::endl;
  builder.solve_collapse_subspace();
  // std::cout << builder.realised_tableau() << std::endl;
...
void ChoiMixBuilder::solve_postselected_subspace() {
  ...
  Circuit diag_circ = mutual_diagonalise(to_diag, diag_ins, cx_config);
  // Extract the dagger of each gate in order from tab
  for (const Command& com : diag_circ) {
    auto args = com.get_args();
...
void ChoiMixBuilder::solve_collapse_subspace() {
  // Input-first gaussian elimination to solve input-sides of remaining rows; we
  // assume this is called after solving post-selected and identity spaces, so
  // each row commutes over the input segment and contains non-trivial output
  // components, and rows with trivial input segment are collected at the bottom
  tab.canonical_column_order(ChoiMixTableau::TableauSegment::Input);
  tab.gaussian_form();
  ...
  for (unsigned r = 0; r < tab.get_n_rows(); ++r) {
    ...
    if (rten.first.string.map.size() == 0) {
      // Reached the rows with empty input segment
      break;
    }
    // Else, we add the row to the subspace
    to_diag_ins.push_back({rten.first, 1.});
  ...
  // All rows in collapsed subspace are now diagonalised so we can just focus on
  // the Z matrix. Reduce them to a minimal set of qubits by first reducing to
  // upper echelon form over the input segment
  std::vector<std::pair<unsigned, unsigned>> row_ops =
      gaussian_elimination_row_ops(tab.tab_.zmat_.topLeftCorner(
          to_diag_ins.size(), input_qubits.size()));
  for (const std::pair<unsigned, unsigned>& op : row_ops) {
    tab.tab_.row_mult(op.first, op.second);
  }
  // Obtain CX instructions as column operations
  std::vector<std::pair<unsigned, unsigned>> col_ops_in =
      gaussian_elimination_col_ops(tab.tab_.zmat_.topLeftCorner(
          to_diag_ins.size(), input_qubits.size()));
  for (const std::pair<unsigned, unsigned>& op : col_ops_in) {
  ...
  std::vector<std::pair<unsigned, unsigned>> col_ops_out =
      gaussian_elimination_col_ops(tab.tab_.zmat_.topRightCorner(
          to_diag_ins.size(), output_qubits.size()));
  for (const std::pair<unsigned, unsigned>& op : col_ops_out) {
    tab.tab_.apply_CX(
        input_qubits.size() + op.second, input_qubits.size() + op.first);
    ChoiMixTableau::col_key_t ctrl =
        tab.col_index_.right.at(input_qubits.size() + op.second);
    ChoiMixTableau::col_key_t trgt =
        tab.col_index_.right.at(input_qubits.size() + op.first);
    out_circ_tp.add_op<Qubit>(OpType::CX, {ctrl.first, trgt.first});
  }

  std::set<Qubit> solved_ins, solved_outs;
  for (unsigned r = 0; r < to_diag_ins.size(); ++r) {
    ...
    solved_ins.insert(in_q);
    solved_outs.insert(out_q);
  }

  // Remove these solved qubits from other rows; we assume that all identity
  // pairs and post-selections have been removed already, so the remaining rows
  // are the initialised subspace; these will still have no input term by the
  // gaussian elimination at the start of this method, but may still contain
  // Z@out_q; perform a mini gaussian elimination over the bottom rows, just on
  // the unsolved qubits, to identify other qubits we can apply CXs into to
  // remove Z@out_q
  unsigned unsolved_rows = tab.get_n_rows() - to_diag_ins.size();
  unsigned unsolved_outs = output_qubits.size() - solved_outs.size();
  MatrixXb unmatched_outs = MatrixXb::Zero(unsolved_rows, 2 * unsolved_outs);
  ...
  for (unsigned out_col = 0; out_col < output_qubits.size(); ++out_col) {
    ...
    if (solved_outs.find(out_qb) != solved_outs.end()) continue;
    unmatched_outs.col(col_lookup.size()) =
        tab.tab_.xmat_.block(to_diag_ins.size(), tab_col, unsolved_rows, 1);
    col_lookup.push_back({out_qb, Pauli::X});
    unmatched_outs.col(col_lookup.size()) =
        tab.tab_.zmat_.block(to_diag_ins.size(), tab_col, unsolved_rows, 1);
  ...
  for (const std::pair<unsigned, unsigned>& op : row_ops) {
    ...
    tab.tab_.row_mult(
        to_diag_ins.size() + op.first, to_diag_ins.size() + op.second);
  }
  // Go through the output-only rows and remove terms from matched qubits
  for (unsigned r = 0; r < unsolved_rows; ++r) {
    unsigned leading_col = 0;
    for (; leading_col < unmatched_outs.cols(); ++leading_col) {
      if (unmatched_outs(r, leading_col)) break;
    }
    std::pair<Qubit, Pauli> alternate_qb = col_lookup.at(leading_col);
    if (alternate_qb.second == Pauli::X) {
      // Make the alternate point of contact Z so we only need one rule for
      // eliminating Paulis
      tab.apply_gate(
    ...
    ChoiMixTableau::row_tensor_t row_paulis =
        tab.get_row(to_diag_ins.size() + r);
    for (const std::pair<const Qubit, Pauli>& qbp :
         row_paulis.second.string.map) {
      if (solved_outs.find(qbp.first) == solved_outs.end()) continue;
      // Z@qbp.first is solved, so remove it by CX with Z@alternate_qb.first
      out_circ_tp.add_op<Qubit>(OpType::CX, {qbp.first, alternate_qb.first});
      tab.apply_CX(
          qbp.first, alternate_qb.first,
          ChoiMixTableau::TableauSegment::Output);
    }
  }

  // Rows Z@in_q, Z@out_q are fully solved and the only rows with content on
  // in_q or out_q; remove the rows and qubits from the tableau; since removing
  // rows/columns replaces them with the row/column from the end, remove in
  // reverse order
  for (unsigned r = to_diag_ins.size(); r > 0;) {
    --r;
    tab.remove_row(r);
  }
  for (auto it = solved_ins.rbegin(); it != solved_ins.rend(); ++it)
...
void ChoiMixBuilder::solve_initialised_subspace() {
  // Canonicalise tableau and perform input-first gaussian elimination to
  // isolate initialised subspace in lower rows
  tab.canonical_column_order(ChoiMixTableau::TableauSegment::Input);
  tab.gaussian_form();

  // Call diagonalisation methods to diagonalise initialised subspace
  std::list<std::pair<QubitPauliTensor, Expr>> to_diag;
  for (unsigned r = tab.get_n_rows(); r > 0;) {
    --r;
    ChoiMixTableau::row_tensor_t rten = tab.get_row(r);
  ...
  unsigned initialised_size = to_diag.size();
  unsigned initialised_start = tab.get_n_rows() - initialised_size;
  qubit_vector_t output_qubits = tab.output_qubits();
  std::set<Qubit> diag_outs{output_qubits.begin(), output_qubits.end()};
  Circuit diag_circ = mutual_diagonalise(to_diag, diag_outs, cx_config);
  // Extract the dagger of each gate in order from tab
  for (const Command& com : diag_circ) {
    auto args = com.get_args();
    out_circ_tp.add_op<UnitID>(com.get_op_ptr()->dagger()->transpose(), args);
  ...
  // Diagonalised rows should still be at the bottom of the tableau - reduce
  // them to a minimal set of qubits for initialisation by first reducing to
  // upper echelon form
  unsigned n_ins = tab.get_n_inputs();
  unsigned n_outs = tab.get_n_outputs();
  std::vector<std::pair<unsigned, unsigned>> row_ops =
      gaussian_elimination_row_ops(
          tab.tab_.zmat_.bottomRightCorner(initialised_size, n_outs));
  for (const std::pair<unsigned, unsigned>& op : row_ops) {
    tab.tab_.row_mult(
        initialised_start + op.first, initialised_start + op.second);
  }
  // Obtain CX instructions as column operations
  std::vector<std::pair<unsigned, unsigned>> col_ops =
      gaussian_elimination_col_ops(
          tab.tab_.zmat_.bottomRightCorner(initialised_size, n_outs));
  // These gates will also swap qubits to isolate the initialisations on the
  // first few qubits - this is fine as can be cleaned up later with peephole
  // optimisations
  for (const std::pair<unsigned, unsigned>& op : col_ops) {
  ...
  // Initialise rows
  for (unsigned r = 0; r < initialised_size; ++r) {
    unsigned final_row = tab.get_n_rows() - 1;
    ChoiMixTableau::row_tensor_t row = tab.get_row(final_row);
