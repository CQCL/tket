...
class BicomponentGraph {
 private:
  ...
  // Map from vertices v (UID_t) to the set of biconnected components of v
  using VertexToCompT = std::map<UID_t, std::set<unsigned>>;

 public:
  /** Construct a biconnected component graph from the graph */
  explicit BicomponentGraph(const UndirectedConnGraph<UID_t>& graph);
...
class BicomponentGraphTester {
 private:
  using UID_t = Node;
  detail::BicomponentGraph<UID_t>* bicomp_g;

 public:
  explicit BicomponentGraphTester(detail::BicomponentGraph<UID_t>* bg)
      : bicomp_g(bg) {}
  const std::vector<bool>& get_selected_comps() const {
    return bicomp_g->selected_comps;
  }
  const std::set<unsigned>& get_comps(UID_t uid) const {
