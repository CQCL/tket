...
QubitPauliTensor& PGRotation::port(unsigned p) {
  if (p != 0) throw PGError("Cannot dereference port on PGRotation: " + std::to_string(p));
  return tensor_;
}
...
QubitPauliTensor& PGCliffordRot::port(unsigned p) {
  if (p != 0) throw PGError("Cannot dereference port on PGCliffordRot: " + std::to_string(p));
  return tensor_;
}
...
QubitPauliTensor& PGMeasure::port(unsigned p) {
  if (p != 0) throw PGError("Cannot dereference port on PGMeasure: " + std::to_string(p));
  return tensor_;
}
...
QubitPauliTensor& PGDecoherence::port(unsigned p) {
  if (p != 0) throw PGError("Cannot dereference port on PGDecoherence: " + std::to_string(p));
  return tensor_;
}
...
QubitPauliTensor& PGReset::port(unsigned p) {
  if (p == 0) return stab_;
  else if (p == 1) return destab_;
  else throw PGError("Cannot dereference port on PGReset: " + std::to_string(p));
}
...
PGOp_ptr PGConditional::symbol_substitution(const SymEngine::map_basic_basic& sub_map) const {
  ...
  if (inner_sub) return std::make_shared<PGConditional>(inner_sub, args_, value_);
...
QubitPauliTensor& PGConditional::port(unsigned p) {
...
PGOp_ptr PGStabilizer::symbol_substitution(const SymEngine::map_basic_basic&) const { return PGOp_ptr(); }
...
std::vector<QubitPauliTensor> PGStabilizer::active_paulis() const { return {stab_}; }

QubitPauliTensor& PGStabilizer::port(unsigned p) {
  if (p != 0) throw PGError("Cannot dereference port on PGStabilizer: " + std::to_string(p));
  return stab_;
}
...
const QubitPauliTensor& PGInputRow::get_input_tensor() const { return input_tensor_; }

SymSet PGInputRow::free_symbols() const { return {}; }

PGOp_ptr PGInputRow::symbol_substitution(const SymEngine::map_basic_basic&) const { return PGOp_ptr(); }
...
std::vector<QubitPauliTensor> PGInputRow::active_paulis() const { return {tensor_}; }

QubitPauliTensor& PGInputRow::port(unsigned p) {
  if (p != 0) throw PGError("Cannot dereference port on PGInputRow: " + std::to_string(p));
  return tensor_;
}
...
const QubitPauliTensor& PGOutputRow::get_output_tensor() const { return output_tensor_; }

SymSet PGOutputRow::free_symbols() const { return {}; }

PGOp_ptr PGOutputRow::symbol_substitution(const SymEngine::map_basic_basic&) const { return PGOp_ptr(); }
...
std::vector<QubitPauliTensor> PGOutputRow::active_paulis() const { return {tensor_}; }

QubitPauliTensor& PGOutputRow::port(unsigned p) {
  if (p != 0) throw PGError("Cannot dereference port on PGOutputRow: " + std::to_string(p));
  return tensor_;
}
...
PauliGraph::PauliGraph() : pauli_ac_(0, 0), pauli_index_(), c_graph_(), qubits_(), bits_(), last_writes_(), last_reads_() {}
  ...
  for (const PGPauli& r_pauli : pauli_index_.get<TagID>()) {
    for (const PGPauli& c_pauli : pauli_index_.get<TagID>()) {
      if (pauli_ac_(r_pauli.index, c_pauli.index)) {
        out << v_map.at(c_pauli.vert) << "_" << c_pauli.port << " -> " << v_map.at(r_pauli.vert) << "_" << r_pauli.port << ";\n";
      }
    }
  }
...
PGVert PauliGraph::add_vertex_at_end(PGOp_ptr op) {
  ...
  pauli_ac_.conservativeResize(mat_offset + op->n_paulis(), mat_offset + op->n_paulis());
  for (unsigned i = 0; i < op->n_paulis(); ++i) {
    pauli_index_.insert({mat_offset + i, v, i});
    pauli_ac_.col(mat_offset + i).setZero();
    for (const std::pair<const Qubit, Pauli>& qp : op->port(i).string.map) qubits_.insert(qp.first);
  }
  ...
  if (op->get_type() == PGOpType::InputRow) {
    ...
    for (const PGPauli& prev_pauli : pauli_index_.get<TagID>()) {
      ...
      if (other_op->get_type() == PGOpType::InputRow) {
        ...
        if (tensor.commutes_with(other_in.get_tensor()) ^ input_tensor.commutes_with(other_in.get_input_tensor())) throw PGError("Cannot insert InputRow into PauliGraph - does not commute with existing InputRow");
      }
      else if (!tensor.commutes_with(other_op->port(prev_pauli.port))) throw PGError("Cannot insert InputRow into PauliGraph - does not commute with existing operations");
    ...
    for (const PGPauli& prev_pauli : pauli_index_.get<TagID>()) {
      ...
      if (other_op->get_type() == PGOpType::OutputRow) {
        const PGOutputRow& other_out = dynamic_cast<const PGOutputRow&>(*other_op);
        if (tensor.commutes_with(other_out.get_tensor()) ^ output_tensor.commutes_with(other_out.get_output_tensor())) throw PGError("Cannot insert OutputRow into PauliGraph - does not commute with existing OutputRow");
        pauli_ac_(mat_offset, prev_pauli.index) = false;
      }
      else {
    ...
    for (unsigned i = 0; i < active.size(); ++i) {
      for (const PGPauli& prev_pauli : pauli_index_.get<TagID>()) {
        ...
        if (other_op->get_type() == PGOpType::OutputRow) {
          if (active.at(i).commutes_with(other_pauli)) throw PGError("Cannot insert operation into PauliGraph - does not commute with existing OutputRow");
          pauli_ac_(mat_offset + i, prev_pauli.index) = false;
        }
        else {
          pauli_ac_(mat_offset + i, prev_pauli.index) = !active.at(i).commutes_with(other_pauli);
        }
      }
    }
  }
  ...
  // If op both reads and writes to the same bit, it may have been inserted into c_preds
...
void PauliGraph::multiply_strings(unsigned source_r, unsigned target_r, Complex coeff) {
  ...
  target_op->port(target_pgp.port) = coeff * source_op->port(source_pgp.port) * target_op->port(target_pgp.port);
  // Update anticommutation matrix
  for (unsigned i = 0; i < pauli_ac_.rows(); ++i) {
    pauli_ac_(i, target_pgp.index) = pauli_ac_(i, source_pgp.index) ^ pauli_ac_(i, target_pgp.index);
    pauli_ac_(target_pgp.index) = pauli_ac_(source_pgp.index, i) ^ pauli_ac_(target_pgp.index, i);
  }
  ...
  while (found_more) {
    ...
    BGL_FORALL_VERTICES(v, c_graph_, PGClassicalGraph) {
      ...
      for (auto it = range.first; it != range.second; ++it) {
        for (const PGPauli& c_pauli : pauli_index_.get<TagID>()) {
          if (pauli_ac_(it->index, c_pauli.index) && (consumed.find(c_pauli.vert) == consumed.end())) {
            initial = false;
            break;
          }
        }
      ...
      found_more = true;
      consumed.insert(v);
      // Check Classical history contains exactly predecessor hazards for all classical bits and all active bits are registered
      PGOp_ptr op = c_graph_[v];
      std::unordered_set<PGVert> justified_preds;
      for (const Bit& b : op->write_bits()) {
        if (bits_.find(b) == bits_.end()) throw PGError("PGOp writes to unregistered bit: " + op->get_name());
        auto w_found = previous_write.find(b);
        if (w_found != previous_write.end()) {
          if (!boost::edge(w_found->second, v, c_graph_).second) throw PGError("No edge in PGClassicalGraph for RAW dependency on bit " + b.repr() + " between " + c_graph_[w_found->second]->get_name() + " and " + op->get_name());
      ...
      for (const Bit& b : op->write_bits()) {
        ...
        auto w_found = previous_write.find(b);
        if (w_found != previous_write.end()) {
          if (!boost::edge(w_found->second, v, c_graph_).second) throw PGError("No edge in PGClassicalGraph for WAW dependency on bit " + b.repr() + " between " + c_graph_[w_found->second]->get_name() + " and " + op->get_name());
          justified_preds.insert(w_found->second);
        }
        auto r_found = previous_reads.find(b);
        if (r_found != previous_reads.end()) {
          for (const PGVert& u : r_found->second) {
            if (!boost::edge(u, v, c_graph_).second) throw PGError("No edge in PGClassicalGraph for WAR dependency on bit " + b.repr() + " between " + c_graph_[u]->get_name() + " and " + op->get_name());
      ...
      for (auto it = in_edge_range.first; it != in_edge_range.second; ++it) {
        ...
        if (justified_preds.find(s) == justified_preds.end()) throw PGError("Edge in PGClassicalGraph despite no dependency between " + c_graph_[s]->get_name() + " and " + op->get_name());
      }
      // Check Pauli history contains all anti-commuting terms and all active qubits are registered
      ...
      for (auto it = range.first; it != range.second; ++it) {
        ...
        for (const std::pair<const Qubit, Pauli>& qp : tensor.string.map) {
          if (qubits_.find(qp.first) == qubits_.end()) throw PGError("PGOp interacts with unregistered qubit: " + op->get_name());
        }
        if (op->get_type() == PGOpType::InputRow) {
          for (unsigned c = 0; c < pauli_ac_.cols(); ++c) {
            if (pauli_ac_(it->index, c)) throw PGError("PauliGraph input row has predecessors in anticommutation matrix: " + op->get_name());
          }
          const PGInputRow& in = dynamic_cast<const PGInputRow&>(*op);
          for (const PGOp_ptr& other : inputs) {
            const PGInputRow& other_in = dynamic_cast<const PGInputRow&>(*other);
            if (in.get_tensor().commutes_with(other_in.get_tensor()) != in.get_input_tensor().commutes_with(other_in.get_input_tensor())) throw PGError("PauliGraph contains anticommuting input rows: " + op->get_name() + " and " + other->get_name());
          }
          inputs.push_back(op);
        }
        else if (op->get_type() == PGOpType::OutputRow) {
          for (const PGPauli& c_pauli : pauli_index_.get<TagID>()) {
            if ((consumed.find(c_pauli.vert) != consumed.end()) && (pauli_ac_(it->index, c_pauli.index) != (c_graph_[it->vert]->get_type() != PGOpType::OutputRow && !tensor.commutes_with(c_graph_[it->vert]->port(it->port))))) throw PGError("PauliGraph anticommutation matrix is missing a link between " + c_graph_[it->vert]->get_name() + " and " + op->get_name());
          }
          for (unsigned r = 0; r < pauli_ac_.rows(); ++r) {
            if (pauli_ac_(r, it->index)) throw PGError("PauliGraph output row has successors in anticommutation matrix: " + op->get_name());
          }
          const PGOutputRow& out = dynamic_cast<const PGOutputRow&>(*op);
          for (const PGOp_ptr& other : outputs) {
            const PGOutputRow& other_out = dynamic_cast<const PGOutputRow&>(*other);
            if (out.get_tensor().commutes_with(other_out.get_tensor()) != out.get_output_tensor().commutes_with(other_out.get_output_tensor())) throw PGError("PauliGraph contains anticommuting output rows: " + op->get_name() + " and " + other->get_name());
          }
          outputs.push_back(op);
        }
        else {
          for (const PGPauli& c_pauli : pauli_index_.get<TagID>()) {
            if ((consumed.find(c_pauli.vert) != consumed.end()) && (pauli_ac_(it->index, c_pauli.index) == tensor.commutes_with(c_graph_[c_pauli.vert]->port(c_pauli.port)))) throw PGError("PauliGraph anticommutation matrix is missing a link between " + c_graph_[c_pauli.vert]->get_name() + " and " + op->get_name());
          }
        }
      }
    }
