...
std::vector<qubit_vector_t> LinePlacement::interactions_to_lines(const Circuit& circ_) {
  qubit_vector_t all_qubits_v = circ_.all_qubits();
  std::vector<WeightedEdge> pattern_weighting = this->default_pattern_weighting(circ_);
  if(pattern_weighting.empty()){
    return {{}};
  }
  const QubitGraph q_graph = this->construct_pattern_graph(pattern_weighting, all_qubits_v.size());
  QubitGraph::UndirectedConnGraph uc_graph = q_graph.get_undirected_connectivity();
  ...
  for (const Qubit& qb : all_qubits_v) {
    if (all_qubits_s.find(qb) != all_qubits_s.end()) found_lines.push_back({qb});
  }
  return found_lines;
}


std::map<Qubit, Node> LinePlacement::assign_lines_to_target_graph(std::vector<qubit_vector_t>&  line_pattern, unsigned n_qubits) {
    unsigned n_unused_nodes = this->architecture_.n_nodes() - n_qubits;
    // Sort lines from longest to shortest
    std::sort(line_pattern.begin(), line_pattern.end(), [](qubit_vector_t x, qubit_vector_t y){
    ...
    // We want to find a useable subgraph of the Architecture for assigning these Qubit 
    // Note that this currently requires removing Nodes from the Architecture
    // so we produce a copy
    // TODO: see TKET-2494
    ...
    std::set_difference(
      all_architecture_nodes.begin(), all_architecture_nodes.end(), bad_nodes.begin(), bad_nodes.end(),
