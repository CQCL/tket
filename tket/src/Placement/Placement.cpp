...
std::map<UnitID, UnitID> Placement::get_placement_map(const Circuit& circ_) const{
    std::vector<std::map<UnitID, UnitID>> all_maps = this->get_all_placement_maps(circ_);
...
QubitGraph::UndirectedConnGraph GraphPlacement::construct_pattern_graph(const std::vector<WeightedEdge>& edges) const {
    QubitGraph q_graph();
