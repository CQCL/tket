...
DerivedGraphsFilter::DerivedGraphsFilter(const FixedData& fixed_data) {
  m_updater_pair_ptr = std::make_unique<DerivedGraphsUpdaterPair>(
      fixed_data.pattern_neighbours_data, fixed_data.target_neighbours_data,
      m_calculator, m_storage);
...
static bool compatible_using_weight_maps(
    VertexWSM pv, VertexWSM tv, DGraph& pattern_graph,
  ...
  return FilterUtils::compatible_sorted_degree_sequences(
...
bool DerivedGraphsFilter::is_compatible(VertexWSM pv, VertexWSM tv) {
  ...
  for (;;) {
    ...
    if (!compatible_using_weight_maps(
            pv, tv, get_derived_pattern_graphs().d2_graph, m_d2_pattern_weights,
            get_derived_target_graphs().d2_graph, m_d2_target_weights)) {
      break;
    }
    if (!compatible_using_weight_maps(
            pv, tv, get_derived_pattern_graphs().d3_graph, m_d3_pattern_weights,
            get_derived_target_graphs().d3_graph, m_d3_target_weights)) {
      break;
    }
