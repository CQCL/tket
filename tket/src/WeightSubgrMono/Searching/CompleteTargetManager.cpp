...
static void fill_pattern_edge_sums(std::map<VertexWSM, WeightWSM>& edge_sums, const NeighboursData& ndata) {
  ...
  for(const auto& entry : map) {
    ...
    //const auto& neighbours_and_weights = ndata.get_neighbours_and_weights(vertex);
...
static void fill_target_cumulative_edge_sums(std::map<VertexWSM, std::vector<WeightWSM>>& edge_sums, const NeighboursData& ndata) {
  const auto& map = ndata.get_map();
  ...
  for(const auto& entry : map) {
    ...
    std::sort(indices.begin(), indices.end(),
          [&neighbours_and_weights](unsigned ii, unsigned jj) {
            return neighbours_and_weights[ii].second < neighbours_and_weights[jj].second;
          });
...
CompleteTargetManager::CompleteTargetManager(const FixedData& fixed_data)
  ...
  fill_pattern_edge_sums(m_pattern_edge_sums, fixed_data.pattern_neighbours_data);
  fill_target_cumulative_edge_sums(m_target_partial_edge_sums, fixed_data.target_neighbours_data);
}



VertexWSM CompleteTargetManager::choose_variable(const SearchNode& node, const Assignments& assignments) const {
...
std::pair<VertexWSM, VertexWSM> CompleteTargetManager::choose_next_assignment(
  ...
  const auto number_of_pv_neighbours = m_fixed_data.pattern_neighbours_data.get_neighbours_and_weights(pv).size();
  ...
  for(auto tv : domain) {
    const auto& tv_sum = m_target_partial_edge_sums.at(tv).at(number_of_pv_neighbours-1);
