...
class RNG {
 public:
   ...
   */
  void set_seed(std::size_t seed = 5489u);
   ...
   * (Although, strictly speaking very far from uniform for larger
   * vectors. The number of permutations grows very rapidly
   * and quickly becomes larger than the total number of distinct states
   * of any fixed engine. Thus, for larger vectors,
   * only a small proportion of permutations are possible).
