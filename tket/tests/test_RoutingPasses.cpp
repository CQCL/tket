...
// SCENARIO("Routing preserves the number of qubits in given instance of CnX
// op") {
...
//   //   }
//   // }
//   GIVEN("A measure operation with bundled out classical edges") {
//     Circuit circ(2, 2);
//     circ.add_measure(0, 0);
//     // circ.add_measure(1, 1);
//     circ.add_conditional_gate<unsigned>(OpType::X, {}, {1}, {0}, 1);
//     circ.add_conditional_gate<unsigned>(OpType::X, {}, {0}, {1}, 1);

//     auto q_frontier = std::make_shared<unit_frontier_t>();
//     for (const Qubit& q : circ.all_qubits()) {
//       q_frontier->insert({q, circ.get_nth_out_edge(circ.get_in(q), 0)});
//     }

//     auto b_frontier = std::make_shared<b_frontier_t>();
//     std::vector<Bit> bits = circ.all_bits();
//     for (const Bit& b : bits) {
//       b_frontier->insert({b, circ.get_all_out_edges(circ.get_in(b))});
//     }

//     for(auto it = b_frontier->get<TagKey>().begin(); it !=
//     b_frontier->get<TagKey>().end(); ++it){
//       std::cout << it->first.repr() << " " << it->second.size() << std::endl;
//     }

//     CutFrontier next_cut = circ.next_cut(q_frontier, b_frontier);

//     REQUIRE(
//         next_cut.b_frontier->get<TagKey>().find(bits[1]) !=
//         next_cut.b_frontier->get<TagKey>().end());
//     REQUIRE(
//         next_cut.b_frontier->get<TagKey>().find(bits[0]) !=
//         next_cut.b_frontier->get<TagKey>().end());
//   }
// }
// SCENARIO(
...
//     add_2qb_gates(circ, OpType::CX, {{0, 1}, {1, 2}, {1, 3}, {1, 4}, {0,
//     1}});
...
//   //     circ.add_conditional_gate<unsigned>(
//   //         OpType::CX, {}, {i + 2, i + 1}, {1, 2, 3, 5, 9}, 0);
//   //     circ.add_conditional_gate<unsigned>(OpType::S, {}, {i + 1}, {1, 2,
//   7}, 1);
//   //     circ.add_conditional_gate<unsigned>(
//   //         OpType::CX, {}, {i, i + 1}, {4, 6, 8, 7, 9}, 0);
//   //     circ.add_conditional_gate<unsigned>(OpType::X, {}, {i + 2}, {0, 3},
//   0);
//   //   }
//   //   MappingManager mm(std::make_shared<Architecture>(arc));
//   //   REQUIRE(mm.route_circuit(
